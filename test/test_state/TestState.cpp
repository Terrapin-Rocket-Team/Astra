...
public:
    ...
    // Test state to return [px, py, pz, vx, vy, vz]
    ...
    void update(Matrix measurements) override {
        // Not used by State anymore
    }

    void updateGPS(double px, double py) override {
        update_gps_called = true;
        last_gps_px = px;
        last_gps_py = py;
    }

    void updateBaro(double pz) override {
        update_baro_called = true;
        last_baro_pz = pz;
    }

    void updateGPSBaro(double px, double py, double pz) override {
...
void test_predict_calls_kalman_filter() {
    ...
    TEST_ASSERT_EQUAL(3, kalmanFilter->last_control.rows());
    TEST_ASSERT_EQUAL(1, kalmanFilter->last_control.cols());
}
