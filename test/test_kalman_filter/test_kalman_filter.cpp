...
// Test-friendly wrapper that exposes protected members
class TestableKalmanFilter : public DefaultKalmanFilter {
public:
    using DefaultKalmanFilter::DefaultKalmanFilter;
...
void test_kalman_measurement_noise_effect(void) {
    ...
    // Give same measurement (far from initial)
    ...
    // Low noise filter should move MORE toward measurement than high noise filter
    double dist1 = sqrt(pow(X1.get(0,0) - 0, 2) + pow(X1.get(1,0) - 0, 2) + pow(X1.get(2,0) - 0, 2));
    double dist2 = sqrt(pow(X2.get(0,0) - 0, 2) + pow(X2.get(1,0) - 0, 2) + pow(X2.get(2,0) - 0, 2));

    TEST_ASSERT_GREATER_THAN(dist2, dist1);
}
