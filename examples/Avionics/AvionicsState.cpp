...
void AvionicsState::determineStage()
{
    ...
    else if (stage == 2 && baroVelocity <= 0 && timeSinceLaunch > 5)
    {
        ...
        LOGI("Apogee detected at %.2f m.", position.z());
        timeOfLastStage = currentTime;
        stage = 3;
        LOGI("Drogue conditions detected.");
    }
    else if (stage == 3 && baro->getASLAltFt() < 1000 && timeSinceLaunch > 10)
    {
        ...
        stage = 4;
        timeOfLastStage = currentTime;
        LOGI("Main parachute conditions detected.");
    }
    else if (stage == 4 && baroVelocity > -1 && baro->getASLAltFt() < 66 && timeSinceLaunch > 15)
    {
        ...
        timeOfLastStage = currentTime;
        stage = 5;
        LOGI("Landing detected. Waiting for 5 seconds to dump data.");
    }
